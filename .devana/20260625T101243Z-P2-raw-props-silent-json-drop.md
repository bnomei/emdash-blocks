...
- 2026-06-27: reopened by audit. Current `RawPropsField.commit()` still accepts literal JSON `null`: `parseJsonDraft("null")` returns `{ ok: true, value: null }`, the empty check skips it, and `typeof result.value !== "object" || Array.isArray(result.value)` does not reject it because `typeof null === "object"`. The path then clears the error and calls `onChange(result.value as BlockBuilderProps)`, allowing `props: null` to be persisted even though `BlockBuilderProps` is `Record<string, unknown>` and the fixed summary promises non-object JSON is rejected. Original syntax-error, array, primitive, and empty-clear examples are blocked; the valid-JSON-null non-object case remains open.
- 2026-06-27: fixed. Added shared `isBlockBuilderProps()` validation plus `parsePropsDraft()` for the raw-props commit decision, and changed `RawPropsField.commit()` to reject any parsed value that is not a non-null object record. Literal `null` now follows the `propsMustBeObject` error path instead of calling `onChange`, while valid object props still commit and empty input still commits `{}`. Regression coverage added for `parsePropsDraft("null")` returning `propsMustBeObject`; `npm test` passes.

DEVANA-KEY: src/admin.tsx:834-837 | P2 | raw-props-silent-json-drop
DEVANA-SUMMARY: Status=fixed | P2 high src/admin.tsx:834-837 - RawPropsField now uses non-null object-record validation, so valid JSON null is rejected with propsMustBeObject instead of persisting props:null; syntax errors, arrays/primitives, and empty clears remain handled.
